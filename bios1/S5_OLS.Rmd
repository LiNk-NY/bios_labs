...
plot_ss(mpg, qsec, mtcars, showSquares = TRUE)
```

```{r}
plot_ss(mpg, qsec, mtcars, showSquares = TRUE, leastSquares = TRUE)
```
...
rownum <- 4
polygon(
  x = c(rep(mtcars$qsec[rownum], 2), rep(mtcars$qsec[rownum] + c(mtcars$mpg[rownum] - fitted(fit1)[rownum]), 2)),
  y = c(fitted(fit1)[rownum], rep(mtcars$mpg[rownum], 2), fitted(fit1)[rownum]),
  border = "red"
)
```
