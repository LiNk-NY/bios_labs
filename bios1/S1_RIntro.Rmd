...
```{r, eval=FALSE}
read.csv("/home/rstudio/data/birthwt.csv")
```

We assign this to a variable / object in order to work with it. 

```{r}
bwt <- read.csv("~/data/birthwt.csv")
```
...
We can either use the subset
```{r}
bwt.first <- subset(bwt, Order == 1)
```

Equivalently, we can use the square brackets to take first borns in the
data:
