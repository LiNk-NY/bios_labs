...
```{r}
fit1 <- glm(trtm ~ height + stent + female + diabetic + acutemi, data = chap3, family = binomial)
summary(fit1)
```
...
```{r}
ps_df <- data.frame(ps_score = predict(fit1, type = "response"), treatment = chap3$trtm)
head(ps_df)
ps_df$treatment <- factor(ps_df$treatment, levels = c(0,1), labels = paste("Treatment type:", c("BMS", "DES")))
```

Using the `lattice` package:

```{r}
histogram(~ps_score | treatment, data = ps_df, type = "count", col = "darkgreen")
```
...
```{r}
library(ggplot2)
ggplot(ps_df, aes(x = ps_score)) + geom_histogram(color = "white") + facet_wrap(~treatment) +
