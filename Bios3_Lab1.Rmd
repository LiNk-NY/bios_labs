...
__Note:__ To run code within the RMarkdown file, you can use the shortcut `Ctrl + Alt + C` to run the code within the current chunk where the cursor is positioned. You can also select `Code` > `Run region` > `Run current chunk` from the RStudio menu. 
